...
function isPalindrome(str) {
  const strFind = str.replace(/[\s.,?! ]/g, '');
  const middle = Math.trunc(strFind.length / 2);
  for (let i = 0; i <= middle; i += 1) {
    if (
      strFind[i].toUpperCase() !== strFind[strFind.length - 1 - i].toUpperCase()
    )
      return false;
  }
  return true;
...
function findLongestWord(sentence) {
  ...
  for (let i = 1; i < arrSentence.length - 1; i += 1) {
    if (arrSentence[pos].length < arrSentence[i].length) pos = i;
  }
  return arrSentence[pos];
}
...
function extractNameFromTemplate(value) {
  return value.slice(value.indexOf(' ', 0) + 1, value.length - 1);
}
...
function encodeToRot13(str) {
  const letter1 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const letter2 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    const pos = letter1.indexOf(str[i]);
    if (pos !== -1) result += letter2[pos];
    else result += str[i];
  }
  return result;
}
